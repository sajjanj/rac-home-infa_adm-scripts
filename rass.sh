#!/bin/bash
# NAME   :rass.sh
# USAGE  :./rass.sh <search_text> <0(exact)|1(like)>
# YODA?  :N
# CRON?  :N
# PMREP  :N 
# DESC   :List source objects that is an exact match or resembles the input search text 
# AUTHOR :Sajjan Janardhanan 10/03/2018

. ~/.bash_profile
func_header "Utility to search INFA Source objects"
...
select * from (
	SELECT '--POBJ | PSUB | PDBNM | PLS' FROM DUAL UNION
	select parent_source_name || ' | ' || parent_subject_area || ' | ' || parent_source_database_name || ' | ' || parent_source_last_saved as LST
	from (
		select * from ${PCMX_SCHEMA}.rep_all_sources 
		where parent_subject_area = subject_area
		and upper(parent_source_name) = upper('${string_pattern}') 
		order by parent_source_name, parent_subject_area, parent_source_database_name)
) order by 1 ;
exit ;
SQLMX
else
...
select * from (
	...
	from (
		select * from ${PCMX_SCHEMA}.rep_all_sources 
		where parent_subject_area = subject_area
		and upper(parent_source_name) like upper('%${string_pattern}%')
		order by parent_source_name, parent_subject_area, parent_source_database_name)
) order by 1 ;
exit ;
SQLMX
